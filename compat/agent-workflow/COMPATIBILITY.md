...
`0.11.5/SNAPSHOT.json` is the current deterministic compatibility capture. It records
...
python scripts/capture-agent-workflow-compat.py \\
  ...
  --output compat/agent-workflow/0.11.5
```
...
- pinned snapshot label: the exact verified Agent-Workflow 0.11.5 source commit recorded in `compatibility.json` and `0.11.5/SNAPSHOT.json`
