...
`0.11.9/SNAPSHOT.json` is the current deterministic compatibility capture. It records
...
python scripts/capture-agent-workflow-compat.py \\
  ...
  --output compat/agent-workflow/0.11.9
```
...
- Agent-Workflow product version: `0.11.9`
- pinned snapshot label: the exact verified Agent-Workflow 0.11.9 source commit recorded in `compatibility.json` and `0.11.9/SNAPSHOT.json`
...
SpecGen `0.2.8` optionally registers `agent-workflow-spec` in the public `agent_workflow.plugins` entry-point group. The adapter imports only `agent_workflow.plugin_api`, requires host version `0.11.9`, and delegates to SpecGen's stable programmatic facade. Canonical schemas are not duplicated as plugin package resources.
