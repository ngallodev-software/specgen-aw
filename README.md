...
**Canonical JSON · append-only decisions · immutable snapshots · traceability · repository evidence · evaluation intent · target compilation · agent skill**
...
SpecGen also ships a thin agent skill at `skills/specgen/SKILL.md` and an optional Agent-Workflow 0.9.0 plugin entry point, `agent-workflow-spec`. The plugin delegates to the same SpecGen API/CLI authorities; it does not create another specification model or make Agent-Workflow a SpecGen runtime dependency.
...
Repository analysis is evidence-first and read-only. It recognizes durable declarations and explicitly referenced evidence; it does not pretend to semantically parse arbitrary source code.
...
For programmatic integrations, `specgen.api` is the stable application facade over validation, authoring readiness/finalization, repository analysis/drift, history, rendering/diffing, evaluation intent, compatibility inspection, and Agent-Workflow compilation.
