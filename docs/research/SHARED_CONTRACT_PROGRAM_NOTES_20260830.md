...
| Installed-wheel conformance proved that SpecGen emits exact bundle declarations only as opaque `workflow.requires` strings. The compiled pack contains no `bundle_provenance`, and Agent-Workflow validates pack manifests from its own local schema rather than negotiating the bundle. Consequently, the native-job check is real but is not reached by a generated prompt pack. | shared-contract conformance blocker | Record `CONTRACT-001`; do not seal TASK-005. Version the prompt-pack contract with structured bundle provenance, make Agent-Workflow validate it through the bundle before preparation, preserve prior pack semantics through a deterministic migration, and prove the installed producer-to-consumer journey. |
| The active SpecGen source-test interpreter does not have the local shared bundle installed, so `python -m pytest tests/test_contract_bundle_adapter.py` raises `ModuleNotFoundError: specgen_contracts`; the source-path test and fresh installed-wheel environment both pass. | development installation gap | Keep the package dependency authoritative; use an isolated wheel install for cross-product evidence and add a documented contributor/bootstrap path before treating bare source-tree tests as self-contained. |
| `agent-run prepare` rejects a `review` role without an explicit launch tier (`acceptance-capable review requires a recorded launch tier`). | CLI learning | Supply the declared task tier (`--tier medium`) for review runs; the preflight created no run or worktree residue. |
